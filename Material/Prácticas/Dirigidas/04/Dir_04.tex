...
%PRESENTACIóN--------------------------------------------------------------------------------	
	\begin{center}
		...
		\textsc{Cálculo de Variaciones II}
		
		Condición de Transversalidad \& Diagrama de Fases
...
%EJERCICIOS---------------------------------------------------------------------------------
	\begin{enumerate}
		...
		\item Diagrama de Fases
				\begin{enumerate}[2a)]
					\item Obtengan, matemáticamente, el diagrama de fases de la siguiente expresión
						\begin{align*}
							\dot{x} & = -x + 2y \\
							\dot{y} & = -3y
						\end{align*}
					\item Se define el siguiente sistema de ecuaciones diferenciales lineales
						\begin{align*}
							...
							\dot{x} & = cy+dx+k
						\end{align*}
					  Dibujar el diagrama de fases según las siguientes condiciones:
						\begin{align*}
							& a>0,\enskip b<0,\enskip c>0,\enskip d>0 & a>0,\enskip b<0,\enskip c<0,\enskip d<0\\
							& a<0,\enskip b>0,\enskip c>0,\enskip d<0 & a=0,\enskip b<0,\enskip c>0,\enskip d=0
						\end{align*}
			...
			\item \textbf{Aplicaciones económicas}
				\begin{enumerate}[3a)]
					\item \textbf{Modelo IS-LM}\\
						...
						Donde $Y$ es el nivel de producción, $r$ es la tasa de interés, $E$ es igual a la suma de los gastos en consumo e inversión, $G$ es el gasto público, $T$ son los pagos por impuestos y $P$ el nivel de precios. Las constantes $a$ y $b$ son positivas y representan la velocidad de ajuste del mercado de bienes y del mercado de dinero, respectivamente.\
					
						Se asume $G,\enskip T,\enskip M$ y $P$ como fijos, además las funciones de gastos $E$ y la demanda por dinero cumplen las siguientes propiedades.
							$$0<E_y<1, \quad E_r<0, \quad L_y>0, \quad L_r<0$$
						Dibujar el diagrama de fases para este modelo y establecer los casos extremos.
					\item \textbf{Interacción Demanda-Oferta}\\
						...
						Asumimos que el precio esperado tiene una tasa de cambio proporcional a su adaptación en el mercado.
							$$\dot{p^e}=\beta (p-p^e), \quad \beta>0 \enskip \textup{constante}$$
					\item \textbf{Explotación óptima de peces}\\
						Suponga que una población de ``$N(t)$"peces en cierto lago, crece a la siguiente tasa:$$\dot{N}(t)=aN(t)-bN^2(t)$$
						En ausencia de actividad de extracción. En una comunidad cercana al lago se consume una cantidad ``$C(t)$" de pescado, que brinda una utilidad igual ``$U(c)$" ($U'(c)>0, \enskip U''(c)<0$) y altera el crecimiento de la biomasa de la siguiente forma:$$\dot{N}(t)=aN(t)-bN^2(t)-C(t)\qquad (a,b>0)$$
						...
						Considerando la población actuaal de pesces $N_0=\frac{a}{b}$, se le pide resolver el siguiente problema de cálculo de variaciones mediante el diagrama de fases.
				\end{enumerate}
			\item Problema integrador\\
					\textbf{Aplicación:\textit{ Modelo de crecimiento de Ramsey-Cass-Koopmans}}
						\begin{align*}
							...
							A-\delta-\rho&<0
						\end{align*}
					Donde $U(c)=\ln c$ y la función de producción $f(k)=Ak$\
